...
+ 更快的启动时间
+ 一致的运行环境
+ 持续交付和部署（ [持续集成(Continuous Integration)](https://en.wikipedia.org/wiki/Continuous_integration)） [持续部署(Continuous Delivery/Deployment)](https://en.wikipedia.org/wiki/Continuous_delivery)
+ 更加轻松的维护和扩展
...
正常情况下里面是空的有一个花括号，里面可放置镜像源

将阿里云的镜像源放进去，我这里用的是阿里云的服务器，放自家的快点
...
# -p 暴露端口  --name容器名称 -d 后台运行
docker run -p 3306:3306 --name mysql -e MYSQL_ROOT_PASSWORD=1233 -d mysql
```
...
```
FROM alpine:3.4RUN addgroup -S redis && adduser -S -G redis redisENTRYPOINT ["docker-entrypoint.sh"]EXPOSE 6379CMD [ "redis-server" ]
```
